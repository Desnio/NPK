...
void mapFile(std::string archivepath)
{
    archives.back().file = CreateFileA(archivepath.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

    LARGE_INTEGER fileSize;
    GetFileSizeEx(archives.back().file, &fileSize);
    archvies.back().size = static_cast<size_t>(fileSize.QuadPart);

    archives.back().mapping = CreateFileMappingA(archives.back().file, NULL, PAGE_READONLY, 0, 0, NULL);

    void* datatemp = MapViewOfFile(archives.back().mapping, FILE_MAP_READ, 0, 0, 0);
...
NPK::NPK(std::string pak_dir)
{
    ...
    for(auto& file : files)
    {
        ...
        std::memcpy(&file.originalsize, data + offset, sizeof(file.originalsize));
        offset += sizeof(file.originalsize);

        std::memcpy(&file.pathsize, data + offset, sizeof(file.pathsize));
        offset += sizeof(file.pathsize);

        std::memcpy(&file.archivepathsize, data + offset, sizeof(file.archivepathsize));
        ...
        std::memcpy(file.archivepath.data(), data + offset, file.archivepath.size());
...
std::vector<char> NPK::LoadFile(std::string filePath)
{
    for(auto& file : files)
    {
        if(file.path == filePath)
        {
            ...
            std::memcpy(file.data.data(), file.archiveptr + file.offset, file.data.size());
