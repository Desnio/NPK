...
void packFolder(const std::filesystem::path& folderPath, const std::filesystem::path& root_Path, int compression_level, int max_archive_size)
{
    ...
    for (auto& entry : std::filesystem::recursive_directory_iterator(folderPath))
    {
        ...
        int result = LZ4_compress_HC(dataUncompressed.data(), data.data(), dataUncompressed.size(), data.size(), compression_level);

        if(result < 0)
        {
            std::cerr << "failed to compress file: " << relativeStr << std::endl;
            continue;
        }
    ...
    std::ofstream of(folderPath.generic_string() + '/' + archivesStr, std::ios::binary);
    if (!of.is_open())
    {
        std::cerr << "failed to make archive: " << archivesStr;
        return;
    }

    std::ofstream Pak_dir(folderPath.generic_string() + '/' + "Pak_dir.npk", std::ios::binary);
    ...
    for (auto& file : files)
    {
        ...
        of.write(file.data.data(), file.data.size());

        std::cout << "tellp = " << of.tellp() << std::endl << "max size = " << max_archive_size;
        if (of.tellp() > max_archive_size)
        {
            ...
            of.open(folderPath.generic_string() + '/' + archivesStr, std::ios::binary);
            if(!of.is_open())
            {
                std::cerr << "failed to make archive: " << archivesStr;
            }
    ...
    for(auto& file : files)
    {
        Pak_dir.write(reinterpret_cast<const char*>(&file.offset), sizeof(file.offset));
        Pak_dir.write(reinterpret_cast<const char*>(&file.size), sizeof(file.size));
        Pak_dir.write(reinterpret_cast<const char*>(&file.originalsize), sizeof(file.originalsize));
        Pak_dir.write(reinterpret_cast<const char*>(&file.pathsize), sizeof(file.pathsize));
        Pak_dir.write(reinterpret_cast<const char*>(&file.archivepathsize), sizeof(file.archivepathsize));
